...
class TestGetUserFiles:
    ...
    def test_admin_forbidden(self, client):
        ...
        response = client.get(
            "/api/v1/files/me", headers={"Authorization": f"Bearer {token}"}
        )
...
class TestDeleteFile:
    ...
    def test_admin_forbidden(self, mock_jwt_decode, client):
        ...
        response = client.delete(
            "/api/v1/files/file1", headers={"Authorization": f"Bearer {token}"}
        )
        assert response.status_code == 403
